...
function fadeintransparency(ins)
	...
	if properties[ins.ClassName] then
		local a = tweenservice:Create(ins, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {[properties[ins.ClassName]] = 0})
		a:Play()
	end
end
function alert(content_, dur)
	...
	wait(0.5)
	local tween = tweenservice:Create(canvansgroup, TweenInfo.new(0.5), {GroupTransparency = 0})
	tween:Play()
	...
	spawn(function()
		local idk3_ = tweenservice:Create(idk3, TweenInfo.new(dur), {Size=UDim2.new(1,0,0,0.3)})
		idk3_:Play()
		idk3_.Completed:Wait()
		idk3.Size  = UDim2.new(1,0,0,0.3)
		local tween = tweenservice:Create(canvansgroup, TweenInfo.new(0.5), {GroupTransparency = 1})
		tween:Play()
